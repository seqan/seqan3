...
Nucleotides are the components of (Deoxy)Ribonucleic acid (DNA/RNA) and contain one of the nucleases
...
1. Construction by character literal, i.e. appending the operator `_dna4` to the respective char symbol. <br>
   This is the handiest way as it can also be used as a temporary object.
...
which is the percentage of nucleases that are either Guanine or Cytosine.
...
1. **Quality alphabet for nucleotides**. The values are produced by sequencing machines and represent the probability
   that a nuclease was recorded incorrectly. The characters are most commonly found in FASTQ files.
   See \ref alphabet_quality for details.
2. **RNA structure alphabets**. They describe RNA nucleases as unpaired or up-/downstream paired and can be found
