...
void load(std::vector<int16_t> & data, seqan3::test::sandboxed_path const& filename)
{
    std::ifstream is(filename, std::ios::binary); // Where input can be found.
...
int main()
{
    ...
    auto tmp_file = tmp.get_path() / "data.out"; // This is a temporary file, use any other filename.
