...
struct structure_file_input_class : public ::testing::Test
{
    ...
    seqan3::test::sandboxed_path create_file(char const * contents, seqan3::test::tmp_directory & tmp)
    {
        auto filename = tmp.get_path() / "structure_file_inupt_constructor.dbn";
...
TEST_F(structure_file_input_class, construct_by_filename)
{
    /* just the filename */
    {
        ...
        auto filename = tmp.get_path() / "structure_file_input_constructor.dbn";
    ...
    {
        seqan3::test::tmp_directory tmp;
        auto filename = tmp.get_path() / "structure_file_input_constructor.xyz";
        std::ofstream filecreator{filename, std::ios::out | std::ios::binary};
    ...
    /* filename + fields */
    {
        ...
        seqan3::test::tmp_directory tmp;
        seqan3::test::sandboxed_path filename = create_file("> ID\nACGU\n....\n", tmp);
...
{
    seqan3::test::tmp_directory tmp;
    auto filename = tmp.get_path() / "empty.dbn";
...
{
    seqan3::test::tmp_directory tmp;
    auto filename = tmp.get_path() / "structure_file_input_test.dbn.gz";
...
{
    seqan3::test::tmp_directory tmp;
    auto filename = tmp.get_path() / "structure_file_input_test.dbn.bgzf";
...
{
    seqan3::test::tmp_directory tmp;
    auto filename = tmp.get_path() / "structure_file_input_test.dbn.bz2";
