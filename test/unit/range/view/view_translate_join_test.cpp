...
using iterator_type = decltype(view::translate_join(std::declval<std::vector<std::vector<seqan3::dna4> > &>()).begin());
...
TYPED_TEST(nucleotide, view_translate)
{
    ...
    auto v1 = vec | view::translate_join;
    ...
    // default parameter translation_frames
    auto v2 = vec | view::translate_join();
    // == [[T,Y,V,R],[R,T,Y,V],[V,R,T],[Y,V,R,T],[T,Y,V,R],[R,T,Y],[S,R,A,L],[R,E,L,*],[E,S,F,S],[A,K,A,L],[L,K,L,S],[*,S,S,R]]
    ...
    auto v3 = vec | view::translate_join(translation_frames::FWD_FRAME_0);
    // == [[T,Y,V,R],[S,R,A,L]]
    ...
    // reverse translation
    auto v4 = vec | view::translate_join(translation_frames::FWD_REV_0);
    ...
    auto v5 = vec | view::translate_join(translation_frames::FWD);
    ...
    auto v6 = vec | view::translate_join(translation_frames::SIX_FRAME);
    // == [[T,Y,V,R],[R,T,Y,V],[V,R,T],[Y,V,R,T],[T,Y,V,R],[R,T,Y],[S,R,A,L],[R,E,L,*],[E,S,F,S],[A,K,A,L],[L,K,L,S],[*,S,S,R]]
    ...
    auto v7 = vec | view::translate_join(translation_frames::FWD_FRAME_0 | translation_frames::FWD_FRAME_2);
    ...
    // function syntax
    auto v8 = view::translate_join(vec, translation_frames::FWD_REV_0);
    // == [[T,Y,V,R],[R,T,Y,V],[V,R,T],[Y,V,R,T],[T,Y,V,R],[R,T,Y],[S,R,A,L], [R,E,L,*], [E,S,F,S], [A,K,A,L], [L,K,L,S], [*,S,S,R]]
    ...
    // combinability
    auto v9 = vec | view::complement | view::translate_join(translation_frames::FWD_REV_0);
    ...
    // combinability
    auto v10 = vec | view::complement | view::translate_join(translation_frames::FWD_REV_0) | std::view::take(1);
    // == [C,M,H,A]
    ...
    auto v11 = detail::view_translate_join(view::complement(vec), translation_frames::FWD_REV_0);
    // == [[C,M,H,A],[M,H,A,C],[S,S,R,N],[R,F,R,E]]
