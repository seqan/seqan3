...
protected:
    ...
    static constexpr auto ungapped_view = view::kmer_hash(ungapped{3});
    static constexpr auto gapped_view = view::kmer_hash(0b101_shape);
...
{
    auto stop_at_t = view::take_until([] (dna4 const x) { return x == 'T'_dna4; });
    ...
    EXPECT_EQ(ungapped2 | std::view::reverse | std::ranges::to<result_t>,
              text2 | ungapped_view | std::view::reverse | std::ranges::to<result_t>);

    EXPECT_EQ(gapped2 | std::view::reverse | std::ranges::to<result_t>,
              text2 | gapped_view | std::view::reverse | std::ranges::to<result_t>);

    EXPECT_EQ(ungapped5 | std::view::reverse | std::ranges::to<result_t>,
              text5 | ungapped_view | std::view::reverse | std::ranges::to<result_t>);

    EXPECT_EQ(gapped5 | std::view::reverse | std::ranges::to<result_t>,
              text5 | gapped_view | std::view::reverse | std::ranges::to<result_t>);
}

TEST_F(kmer_hash_test, invalid_sizes)
{
    EXPECT_THROW(text1 | view::kmer_hash(ungapped{33}), std::invalid_argument);
    EXPECT_THROW(text1 | std::view::reverse | view::kmer_hash(ungapped{33}), std::invalid_argument);
}
