...
TEST(view_get, basic)
{
    ...
    //functor
    dna4_vector functor0 = views::get<0>(qv) | std::ranges::to<std::vector>;
    std::vector<phred42> functor1 = views::get<1>(qv) | std::ranges::to<std::vector>;
    EXPECT_EQ(cmp0, functor0);
    EXPECT_EQ(cmp1, functor1);

    // pipe notation
    dna4_vector pipe0 = qv | views::get<0> | std::ranges::to<std::vector>;
    std::vector<phred42> pipe1 = qv | views::get<1> | std::ranges::to<std::vector>;
    EXPECT_EQ(cmp0, pipe0);
    EXPECT_EQ(cmp1, pipe1);

    // combinability
    dna4_vector cmp2{"TGCA"_dna4};
    dna4_vector comp = qv | views::get<0> | views::complement | std::ranges::to<std::vector>;
    EXPECT_EQ(cmp2, comp);

    std::string cmp3{"TGCA"};
    std::string to_char_test = comp | views::to_char | std::ranges::to<std::string>;
...
TEST(view_get, advanced)
{
    ...
    std::vector<masked<dna4>> functor0 = views::get<0>(t) | std::ranges::to<std::vector>;
    EXPECT_EQ(cmp0, functor0);

    std::vector<phred42> cmp1{phred42{0}, phred42{1}, phred42{2}, phred42{3}};
    std::vector<phred42> functor1 = views::get<1>(t) | std::ranges::to<std::vector>;
    EXPECT_EQ(cmp1, functor1);

    std::vector<dna4> cmp00{'A'_dna4, 'C'_dna4, 'G'_dna4, 'T'_dna4};
    std::vector<dna4> functor00 = views::get<0>(views::get<0>(t)) | std::ranges::to<std::vector>;
    EXPECT_EQ(cmp00, functor00);

    // pipe notation
    std::vector<masked<dna4>> pipe0 = t | views::get<0> | std::ranges::to<std::vector>;
    EXPECT_EQ(cmp0, pipe0);

    std::vector<phred42> pipe1 = t | views::get<1> | std::ranges::to<std::vector>;
    EXPECT_EQ(cmp1, pipe1);

    std::vector<dna4> pipe00 = t | views::get<0> | views::get<0> | std::ranges::to<std::vector>;
    ...
    std::vector<masked<dna4>> revtest = t | views::get<0> | std::views::reverse | std::ranges::to<std::vector>;
    EXPECT_EQ(cmprev, revtest);

    std::vector<dna4> cmprev2{'T'_dna4, 'G'_dna4, 'C'_dna4, 'A'_dna4};
    std::vector<dna4> revtest2 = t | views::get<0> | views::get<0> | std::views::reverse | std::ranges::to<std::vector>;
...
TEST(view_get, tuple_pair)
{
    ...
    std::vector<int> pair_func = views::get<0>(pair_test) | std::ranges::to<std::vector>;
    std::vector<int> tuple_func = views::get<0>(tuple_test) | std::ranges::to<std::vector>;
    ...
    // pipe notation
    cmp[0] = 0;
    std::vector<int> pair_pipe = pair_test | views::get<0> | std::ranges::to<std::vector>;
    std::vector<int> tuple_pipe = tuple_test | views::get<0> | std::ranges::to<std::vector>;
...
TEST(view_get, nested_zip_view)
{
    ...
    auto get_view = std::views::zip(std::views::zip(vec1, vec1), vec1) | views::get<0>;
