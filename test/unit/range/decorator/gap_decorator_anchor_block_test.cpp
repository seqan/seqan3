...
#include <seqan3/range/decorator/gap_decorator_anchor_block.hpp>
...
using decorator_t = gap_decorator_anchor_block<std::vector<dna4> const &>;

const std::vector<dna4> dummy_obj{}; // dummy lvalue for type declaration of views
using decorator_t2 = gap_decorator_anchor_block<
...
class aligned_sequence<gap_decorator_anchor_block<inner_type_>> : public ::testing::Test
{
public:
...
INSTANTIATE_TYPED_TEST_CASE_P(gap_decorator_anchor_block, aligned_sequence, test_types);
...
template <typename t>
class gap_decorator_anchor_block_f : public ::testing::Test {};

TYPED_TEST_CASE(gap_decorator_anchor_block_f, test_types);

// concept checks
TYPED_TEST(gap_decorator_anchor_block_f, concept_checks)
{
...
TYPED_TEST(gap_decorator_anchor_block_f, construction_general)
{
    // default
    {
...
TEST(gap_decorator_anchor_block, construction_from_ungapped_sequence)
{
    {
        ...
        // non-const version
        gap_decorator_anchor_block dec{v};
        EXPECT_EQ('A'_dna4, dec[0]);
        EXPECT_EQ('C'_dna4, dec[1]);

        // const version
        gap_decorator_anchor_block const dec2(v_const);
...
TEST(gap_decorator_anchor_block, assignment_from_ungapped_sequence)
{
    {
...
TEST(gap_decorator_anchor_block, comparison)
{
    std::vector<dna4> v{"ACTG"_dna4};

    gap_decorator_anchor_block dec{v};
    gap_decorator_anchor_block dec2{v};
    ...
    gap_decorator_anchor_block decNE{v2};
    EXPECT_NE(dec, decNE);
}


TEST(gap_decorator_anchor_block, begin_and_end)
{
    std::vector<dna4> v{"ACTG"_dna4};

    gap_decorator_anchor_block dec{v};
    gap_decorator_anchor_block const dec_const{v};
...
TEST(gap_decorator_anchor_block, gap_decorator_anchor_block_iterator)
{
    ...
    // random access through gap_decorator_anchor_block operator[]
...
TEST(gap_decorator_anchor_block, decorator_on_views)
{
    std::vector<dna4> v{"ACTG"_dna4};

    auto sub = std::ranges::subrange<decltype(v.begin()), decltype(v.begin())>{v.begin()+1, v.begin()+3};
    gap_decorator_anchor_block dec{sub};
    ...
    gap_decorator_anchor_block dec2{v | view::to_char};
