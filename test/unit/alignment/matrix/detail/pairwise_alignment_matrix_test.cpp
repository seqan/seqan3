...
using matrix_t = seqan3::detail::pairwise_alignment_matrix<score_matrix_t, trace_matrix_t>;
...
struct iterator_fixture<matrix_iterator_t> : public ::testing::Test
{
    ...
    static void expect_eq(actual_column_t && actual_column, expected_column_t && expected_column)
    {
        ...
        for (; actual_it != actual_column.end(); ++actual_it, ++expected_it)
        {
            ...
            EXPECT_EQ(actual_cell.optimal_score(), std::get<0>(std::get<0>(expected_cell)));
            ...
            EXPECT_EQ(actual_cell.trace(), std::get<0>(std::get<1>(expected_cell)));
