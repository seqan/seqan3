...
{
    EXPECT_TRUE(run_test(align_cfg::edit | align_cfg::result{align_cfg::with_back_coordinate}));
}

TEST(alignment_configurator, configure_edit_begin_position)
{
    EXPECT_TRUE(run_test(align_cfg::edit | align_cfg::result{align_cfg::with_front_coordinate}));
}

TEST(alignment_configurator, configure_edit_trace)
{
    EXPECT_TRUE(run_test(align_cfg::edit | align_cfg::result{align_cfg::with_alignment}));
}
...
TEST(alignment_configurator, configure_affine_global_end_position)
{
    auto cfg = align_cfg::mode{global_alignment} |
               align_cfg::gap{gap_scheme{gap_score{-1}, gap_open_score{-10}}} |
               align_cfg::scoring{nucleotide_scoring_scheme{}} |
               align_cfg::result{align_cfg::with_back_coordinate};
...
TEST(alignment_configurator, configure_affine_global_begin_position)
{
    auto cfg = align_cfg::mode{global_alignment} |
               align_cfg::gap{gap_scheme{gap_score{-1}, gap_open_score{-10}}} |
               align_cfg::scoring{nucleotide_scoring_scheme{}} |
               align_cfg::result{align_cfg::with_front_coordinate};
...
TEST(alignment_configurator, configure_affine_global_banded_with_alignment)
{
    ...
    auto cfg_trace = cfg | align_cfg::result{align_cfg::with_alignment};
    auto cfg_begin = cfg | align_cfg::result{align_cfg::with_front_coordinate};
    auto cfg_end = cfg | align_cfg::result{align_cfg::with_back_coordinate};
