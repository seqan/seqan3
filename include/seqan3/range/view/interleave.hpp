...
public:
    ...
    /*!\brief Construct from a viewable_range urange and inserted_range by wrapping in a view::all.
    ...
    template <typename orng_t, typename oirng_t>
        //!\cond
        requires std::constructible_from<urng_t, decltype(view::all(std::declval<orng_t>()))> &&
                 std::constructible_from<inserted_rng_t, decltype(view::persist(std::declval<oirng_t>()))>
        //!\endcond
...
struct interleave_fn
{
    ...
    constexpr auto operator()(urng_t && urange, size_type const size, inserted_rng_t && i) const noexcept
    {
        static_assert(std::ranges::forward_range<urng_t>,
            "The underlying range parameter in view::interleave must model std::ranges::forward_range.");
        static_assert(std::ranges::viewable_range<urng_t>,
            "The underlying range parameter in view::interleave must model std::ranges::viewable_range.");
        static_assert(std::ranges::forward_range<inserted_rng_t>,
            "The range to be inserted by view::interleave must model std::ranges::forward_range.");
