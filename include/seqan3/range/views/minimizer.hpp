 ...
 * \brief Provides seqan3::views::minimizer.
 */
...
/*!\brief The type returned by seqan3::views::minimizer.
...
class minimizer : public std::ranges::view_interface<minimizer<urng_t>>
{
private:
    static_assert(std::ranges::forward_range<urng_t const>, "The minimizer only works on forward_ranges");
...
public:
    ...
    minimizer(minimizer const & rhs)             = default; //!< Defaulted.
    minimizer(minimizer && rhs)                  = default; //!< Defaulted.
    minimizer & operator=(minimizer const & rhs) = default; //!< Defaulted.
    minimizer & operator=(minimizer && rhs)      = default; //!< Defaulted.
    ~minimizer()                                 = default; //!< Defaulted.

    //!\brief Construct from a view and a given window_size.
    minimizer(urng_t urange_, uint32_t const & w_) : urange{std::move(urange_)}, window_size{w_}{}

    //!\brief Construct from a non-view that can be view-wrapped and a given window_size.
    template <typename rng_t>
    //!\cond
     requires !std::same_as<seqan3::remove_cvref_t<rng_t>, minimizer> &&
              std::ranges::viewable_range<rng_t> &&
              std::constructible_from<urng_t, ranges::ref_view<std::remove_reference_t<rng_t>>>
    //!\endcond
    minimizer(rng_t && urange_, uint32_t const & k_, uint32_t const & w_) :
...
template <std::ranges::view urng_t>
template <typename rng_t>
class minimizer<urng_t>::window_iterator
{
private:
...
public:
    ...
    */
    ...
    /*!\brief Move the iterator by a given offset and return the corresponding minimizer.
     * \attention This function is only avaible if `it_t` models std::random_access_iterator.
     */
...
private:
    ...
    //!brief Keeps track if the minimizer value changed.
    bool minimizer_changed{false};

    //!\brief The minimizer value.
    size_t minimizer_value{0};
    ...
    void get_minimizer()
    {
        ...
        {
            // Call next_minimizer until minimizer value changed or end of the underlying range is reached.
            while((!minimizer_changed) && (window_right < last_elem))
            {
                next_minimizer();
            }
     ...
     * \attention This function is only avaible if `it_t` models std::random_access_iterator.
     */
    void get_minimizer(difference_type const skip)
    //!\cond
        requires std::random_access_iterator<it_t>
    //!\endcond
    {
    ...
    //!\brief Calculates minimizers for first window.
    void window_first()
    {
        ...
        windowValues.push_back(*window_right);
        minimizer_value = *(std::min_element(std::begin(windowValues), std::end(windowValues)));
    }

    //!\brief Calculates the next minimizer value via rolling hash.
    ...
    void next_minimizer()
    {
        ...
        std::ranges::advance(window_left, 1);
        std::ranges::advance(window_right, 1);
        if (minimizer_value == *(std::begin(windowValues)))
        {
        ...
        if ((windowValues.back() < minimizer_value))
        {
            minimizer_value = *(std::end(windowValues) - 1);
            minimizer_changed = true;
        }
...
minimizer(rng_t &&, uint32_t const & window_size) ->
minimizer<std::ranges::all_view<rng_t>>;


// ---------------------------------------------------------------------------------------------------------------------
// minimizer_fn (adaptor definition)
// ---------------------------------------------------------------------------------------------------------------------
...
struct minimizer_fn
{
    ...
    constexpr auto operator()(urng_t && urange, uint32_t const & window_size) const
    {
        ...
        return minimizer{urange, window_size};
    }
};
//![adaptor_def]
...
/*!\brief               Computes minimizers for each position of a range for a given window size.
 *                      The parameter window size is optional.
 * \tparam urng_t       The type of the range being processed. See below for requirements. [template parameter is
 *                      omitted in pipe notation]
 * \param[in] urange    The range being processed. [parameter is omitted in pipe notation]
 * \returns             A range of std::size_t where each value is the hash of the resp. minimizer.
 ...
 * \hideinitializer
 */
inline constexpr auto minimizer = detail::minimizer_fn{};
