 ...
 * \tparam urng_t The type of the underlying ranges, must model std::ranges::forward_range, the reference type must
...
public:
    ...
    */
    minimiser_view(urng_t urange_, size_t const w_) : urange{std::move(urange_)}, window_values_size{w_}{}
...
private:
    ...
    using urng_iterator_t = std::ranges::iterator_t<rng_t>;
    //!\brief The sentinel type of the underlying range.
    using urng_sentinel_t = std::ranges::sentinel_t<rng_t>;
...
public:
    ...
    using difference_type = typename std::ranges::range_difference_t<rng_t>;
    //!\brief Value type of this iterator.
    using value_type = std::ranges::range_value_t<rng_t>;
    ...
    * \param[in] urng_iterator             Iterator pointing to the first position of the std::totally_ordered range.
    * \param[in] urng_sentinel             Iterator pointing to the last position of the std::totally_ordered range.
    ...
    */
    ...
    {
        return (lhs.urng_iterator == rhs.urng_iterator) &&
    ...
    friend bool operator<(window_iterator const & lhs, window_iterator const & rhs)
    {
        return (lhs.urng_iterator < rhs.urng_iterator) && (lhs.window_values.size() < rhs.window_values.size());
    }
...
private:
    ...
    void window_first(size_t const window_values_size)
    {
        ...
        {
            window_values.push_back(*urng_iterator);
            ++urng_iterator;
        }
        window_values.push_back(*urng_iterator);
    ...
    bool next_minimiser()
    {
        ...
        value_type const new_value = *urng_iterator;
...
minimiser_view(rng_t &&, size_t const & window_values_size) -> minimiser_view<std::views::all_t<rng_t>>;
...
struct minimiser_fn
{
    ...
    constexpr auto operator()(urng_t && urange, size_t const window_values_size) const
    {
