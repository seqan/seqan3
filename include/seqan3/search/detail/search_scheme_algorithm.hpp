...
public:
     ...
     * \details
     *
     * The indexed query parameter is a pair of an index and a query which shall be searched in the index.
    ...
    //!\cond
        requires (std::tuple_size_v<indexed_query_t> == 2) &&
                 ...
                 std::invocable<callback_t, std::pair<std::tuple_element_t<0, indexed_query_t>, search_result_type>>
    //!\endcond
    void operator()(indexed_query_t && indexed_query, callback_t && callback) noexcept
    {
        ...
        // Invoke the callback on the received result.
        for (auto && search_result : this->make_results(std::move(internal_hits))) // see policy_search_result_builder
            callback(std::pair{query_idx, search_result});
    }
