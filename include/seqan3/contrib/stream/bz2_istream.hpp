// SPDX-FileCopyrightText: 2003 Jonathan de Halleux
...
size_t const BZ2_INPUT_DEFAULT_BUFFER_SIZE = 4096;
...
class basic_bz2_istreambuf : public std::basic_streambuf<Elem, Tr>
{
public:
...
basic_bz2_istreambuf<Elem, Tr, ElemA, ByteT, ByteAT>::basic_bz2_istreambuf(istream_reference istream_,
                                                                           size_t verbosity_,
                                                                           bool small_,
                                                                           size_t read_buffer_size_,
                                                                           size_t input_buffer_size_) :
    ...
    m_buffer(read_buffer_size_)
{
    ...
    m_err = BZ2_bzDecompressInit(&m_bzip2_stream, std::min(4, static_cast<int>(verbosity_)), static_cast<int>(small_));
...
template <typename Elem, typename Tr, typename ElemA, typename ByteT, typename ByteAT>
size_t basic_bz2_istreambuf<Elem, Tr, ElemA, ByteT, ByteAT>::fill_input_buffer()
{
...
template <typename Elem, typename Tr, typename ElemA, typename ByteT, typename ByteAT>
void basic_bz2_istreambuf<Elem, Tr, ElemA, ByteT, ByteAT>::put_back_from_bzip2_stream()
{
    if (m_bzip2_stream.avail_in == 0)
        return;

    m_istream.clear(std::ios::goodbit);
    m_istream.seekg(-static_cast<int>(m_bzip2_stream.avail_in), std::ios_base::cur);

    m_bzip2_stream.avail_in = 0;
}

template <typename Elem, typename Tr, typename ElemA, typename ByteT, typename ByteAT>
basic_bz2_istreambuf<Elem, Tr, ElemA, ByteT, ByteAT>::~basic_bz2_istreambuf()
{
    BZ2_bzDecompressEnd(&m_bzip2_stream);
}

template <typename Elem, typename Tr, typename ElemA, typename ByteT, typename ByteAT>
typename basic_bz2_istreambuf<Elem, Tr, ElemA, ByteT, ByteAT>::int_type
basic_bz2_istreambuf<Elem, Tr, ElemA, ByteT, ByteAT>::underflow()
{
    ...
    int num = unbzip2_from_stream(&(m_buffer[0]) + 4,
...
template <typename Elem, typename Tr, typename ElemA, typename ByteT, typename ByteAT>
std::streamsize basic_bz2_istreambuf<Elem, Tr, ElemA, ByteT, ByteAT>::unbzip2_from_stream(char_type * buffer_,
...
public:
    typedef std::basic_istream<Elem, Tr> & istream_reference;
    typedef basic_bz2_istreambuf<Elem, Tr, ElemA, ByteT, ByteAT> unbzip2_streambuf_type;

    basic_bz2_istreambase(istream_reference ostream_,
                          size_t verbosity_,
                          bool small_,
                          size_t read_buffer_size_,
                          size_t input_buffer_size_) :
...
{
public:
    typedef basic_bz2_istreambase<Elem, Tr, ElemA, ByteT, ByteAT> bzip2_istreambase_type;
    typedef std::basic_istream<Elem, Tr> istream_type;
    typedef istream_type & istream_reference;
    typedef unsigned char byte_type;

    basic_bz2_istream(istream_reference istream_,
                      size_t verbosity_ = 0,
                      bool small_ = false,
