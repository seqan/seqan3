...
public:
    ...
    structure_file_output(stream_t & stream,
        ...
        format{detail::structure_file_output_format<file_format, stream_char_type>{*secondary_stream}}
    {
...
protected:
    ...
    using format_type = typename detail::variant_from_tags<valid_formats,
                                                           detail::structure_file_output_format,
                       ...
                       offset_type && offset)
    {
        ...
        std::visit([&] (auto & f)
        {
            ...
            else
            {
                ...
                for (auto && v : zipped)
                {
                    f.write(options, std::get<0>(v), std::get<1>(v), std::get<2>(v), std::get<3>(v),
