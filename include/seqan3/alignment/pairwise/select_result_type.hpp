...
/*!\file
 * \brief Provides seqan3::detail::select_result_type.
...
struct select_result_type
{
    ...
    static constexpr auto _determine()
    {
        ...
        {
            if constexpr (configuration_t::template exists<align_cfg::result<with_end_position_type>>())
                return align_result<type_list<uint32_t,
                                              score_type,
                                              alignment_coordinate>>{};
            else if constexpr (configuration_t::template exists<align_cfg::result<with_begin_position_type>>())
                return align_result<type_list<uint32_t,
                                              score_type,
                                              alignment_coordinate,
                                              alignment_coordinate>>{};
            else if constexpr (configuration_t::template exists<align_cfg::result<with_trace_type>>())
                return align_result<type_list<uint32_t,
                                              score_type,
                                              alignment_coordinate,
                                              alignment_coordinate,
                                              std::pair<std::vector<gapped<first_seq_value_type>>,
                                                        std::vector<gapped<second_seq_value_type>>>>>{};
            else
