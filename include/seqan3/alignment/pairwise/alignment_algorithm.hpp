...
public:
        ...
        requires is_banded
    {
        ...
        auto [trimmed_first_range, trimmed_second_range] = this->trim_sequences(first_range, second_range, band);

        is_fst_range_empty = std::ranges::empty(trimmed_first_range);
        is_snd_range_empty = std::ranges::empty(trimmed_second_range);

        auto cache = this->make_cache(
            cfg_ptr->template value_or<align_cfg::gap>(gap_scheme{gap_score{-1}, gap_open_score{-10}}));
        // In the allocation we need to trim
        // But then the builder needs the sequences
        this->allocate_matrix(trimmed_first_range, trimmed_second_range, band, cache);
        ...
        return alignment_result{make_result(idx, trimmed_first_range, trimmed_second_range, cache)};
    }
...
private:
    ...
    void initialise_matrix(cache_t & cache)
    {
        ...
        auto zip_column = std::views::zip(*this->score_matrix_iter, *this->trace_matrix_iter);
    ...
    void compute_matrix(first_range_t & first_range,
                        ...
                        cache_t & cache)
    {
        using zip_column_t = decltype(std::views::zip(*this->score_matrix_iter, *this->trace_matrix_iter));
        ...
        ranges::for_each(first_range, [&, this] (auto const seq1_value)
        {
            // Move to the next column in the alignment matrix and initialise first cell.
            zip_column_t zip_column = std::views::zip(*++this->score_matrix_iter, *++this->trace_matrix_iter);
        ...
        if (is_snd_range_empty) // TODO [[unlikely]]
        {
            this->check_score_last_column_or_cell(
                std::views::zip(*this->score_matrix_iter, *this->trace_matrix_iter).front(), cache);
        }
        else
        {
            this->check_score_last_column_or_cell(*column_iter, cache);
                               ...
                               cache_t & cache)
    {
        ...
        column_iter_t column_iter{};
        ranges::for_each(first_range | views::take_exactly(this->score_matrix.band_col_index),
        ...
        ranges::for_each(first_range | views::drop(this->score_matrix.band_col_index),
        [&, this] (auto first_range_value)
        {
            // Prepare next column.
            zip_column_t zip_column = std::views::zip(*++this->score_matrix_iter, *++this->trace_matrix_iter);
     ...
     *
     * \returns A new alignment result with the requested values.
     *
     * \details
     *
     *
    ...
    auto make_result(index_t index, fst_range_t & fst_range, snd_range_t & snd_range, cache_t const & cache)
    {
        ...
        using result_config_type = std::remove_reference_t<decltype(
                seqan3::get<align_cfg::result>(std::declval<config_t>()).value)>;

        // Choose what needs to be computed.
        if constexpr (result_config_type::rank >= 0)  // compute score
            res.score = cache.optimum.score;

        if constexpr (result_config_type::rank >= 1)  // compute back coordinate
        {
        ...
        if constexpr (result_config_type::rank >= 2) // compute front coordinate
        {
            // Get a aligned sequence builder for banded or un-banded case.
            aligned_sequence_builder builder{fst_range, snd_range};

            if constexpr (is_banded)
                builder.set_band_column_offset(this->trace_matrix.band_col_index);

            auto trace_res = builder(this->trace_matrix.trace_path(cache.optimum.coordinate));
            res.front_coordinate.first = trace_res.begin.col;
            res.front_coordinate.second = trace_res.begin.row;

            if constexpr (result_config_type::rank == 3) // compute alignment
    ...
    /*!\brief Dumps the current alignment matrix in the debug score and if requested trace matrix.
     * \param[in] column The current column.
     *
     * \details
     *
     * Copies the current score and if configured the curent trace column into the local debug score and trace matrix.
