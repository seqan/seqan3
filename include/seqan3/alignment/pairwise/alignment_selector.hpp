...
struct alignment_configurator
{
    ...
    {
        using first_seq_t  = std::remove_reference_t<std::tuple_element_t<0, remove_cvref_t<decltype(*seqan3::begin(seq_range))>>>;
        using second_seq_t = std::remove_reference_t<std::tuple_element_t<1, remove_cvref_t<decltype(*seqan3::begin(seq_range))>>>;

        using result_t = align_result<typename select_result_type<first_seq_t, second_seq_t, remove_cvref_t<config_t>>::type>;
        ...
        auto const & scoring_scheme = cfg.template value_or<align_cfg::scoring>(
                nucleotide_scoring_scheme{match_score{0}, mismatch_score{-1}});
        // Linear gaps
