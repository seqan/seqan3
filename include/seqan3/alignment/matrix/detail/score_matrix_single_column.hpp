 ...
 * This score matrix scores a column for the optimal score and the horizontal score and uses a virtual column that
 * stores only one value of type `score_t` for the vertical column. Hence, this matrix can only be used for a column
...
public:
    ...
    /*!\brief Resets the matrix and reallocates the memory of the underlying matrix.
    ...
    void reset_matrix(sequence1_t && sequence1, sequence2_t && sequence2)
    {
        ...
        m_optimal_column.resize(number_of_rows, 0);
        m_horizontal_column.resize(number_of_rows, 0);
 ...
 * \details
 *
 * Implements a counted iterator to simulate the iteration over the actual matrix. When dereferencing the
 * iterator returns a view over the allocated memory for the respective columns. The returned view is zips
 * the three columns into a single range and transforms the returned tuple with the references to a
...
private:
    ...
    //!\brief The current column stored as a position.
